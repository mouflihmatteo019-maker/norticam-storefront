...
export function ProductCard({ product, prominent = false }: { product: Product; prominent?: boolean }) {
  ...
  return <article className={`group relative overflow-hidden rounded-[1.65rem] border border-slate-200/80 bg-white p-3 shadow-[0_8px_30px_rgb(15,23,42,0.05)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_44px_rgb(15,23,42,0.10)] ${prominent ? "md:p-4" : ""}`}>
    ...
    <div className="px-1 pb-1 pt-4"><p className="mb-1 text-[10px] font-bold uppercase tracking-[0.15em] text-[#1672d8]">NORTICAM</p><Link href={`/produits/${product.handle}`}><h3 className="font-display text-base font-extrabold tracking-tight text-slate-950 sm:text-lg">{product.shortTitle}</h3></Link><p className="mt-2 min-h-10 text-sm leading-5 text-slate-500">{product.description}</p><div className="mt-4 flex items-center justify-between gap-3"><span className="font-display text-lg font-extrabold text-slate-950">{euro(product.price)}</span><button type="button" onClick={() => add(product)} className="inline-flex h-10 items-center gap-1.5 rounded-full bg-slate-950 px-3.5 text-xs font-bold text-white transition hover:bg-[#1672d8] active:scale-95" aria-label={`Ajouter ${product.title} au panier`}><Plus size={15} /> Ajouter</button></div></div>
  </article>;
}
