import { DecisionLinks } from "@/components/DecisionLinks";
...
export function GuideArticle() {
  ...
  if (!article) return <StorefrontLayout><SEOHead title="Guide introuvable | NORTICAM" description="Retrouvez nos guides dashcam voiture et moto." noindex /><div className="container py-24"><h1 className="font-display text-4xl font-extrabold">Guide introuvable</h1><Link href="/conseils" className="mt-6 inline-flex rounded-full bg-slate-950 px-5 py-3 text-sm font-bold text-white">Voir tous les conseils</Link></div></StorefrontLayout>;
  const articleSchema = { "@context": "https://schema.org", "@type": "Article", headline: article.title, description: article.description, author: { "@type": "Organization", name: "NORTICAM" }, publisher: { "@type": "Organization", name: "NORTICAM" }, mainEntityOfPage: `${SITE_URL}/conseils/${slug}/` };
  const unusedFaqSchema = { "@context": "https://schema.org", "@type": "FAQPage", mainEntity: article.checklist.map((item) => ({ "@type": "Question", name: item, acceptedAnswer: { "@type": "Answer", text: "Consultez les informations de la fiche produit et vérifiez la configuration adaptée à votre usage avant achat." } })) };
  return <StorefrontLayout><SEOHead title={`${article.title} | NORTICAM`} description={article.description} type="article" jsonLd={articleSchema} /><article><header className="relative overflow-hidden bg-slate-950 py-14 text-white sm:py-20"><div className="absolute inset-0 bg-[radial-gradient(circle_at_75%_30%,rgba(36,131,230,.3),transparent_25%)]" /><div className="container relative max-w-4xl"><Link href="/conseils" className="inline-flex items-center gap-2 text-sm font-bold text-slate-300 hover:text-white"><ArrowLeft size={16} />Retour aux conseils</Link><p className="mt-9 eyebrow text-[#75b8ff]">{article.category}</p><h1 className="mt-4 font-display text-4xl font-extrabold leading-[1.02] tracking-[-.055em] sm:text-6xl">{article.title}</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-slate-300">{article.intro}</p></div></header><div className="container max-w-4xl py-12 sm:py-20"><div className="grid gap-12 lg:grid-cols-[1fr_290px]"><div className="space-y-12">{article.sections.map(([heading, copy], index) => <section key={heading}><p className="eyebrow">0{index + 1} — À vérifier</p><h2 className="mt-3 font-display text-3xl font-extrabold tracking-[-.04em] text-slate-950">{heading}</h2><p className="mt-5 text-base leading-8 text-slate-600">{copy}</p></section>)}<section className="rounded-[2rem] bg-[#eaf3ff] p-7 sm:p-9"><p className="eyebrow">Une prochaine étape claire</p><h2 className="mt-3 font-display text-3xl font-extrabold tracking-[-.04em] text-slate-950">Passez de la question au bon modèle.</h2><p className="mt-4 leading-7 text-slate-600">Le catalogue permet de comparer les configurations et les variantes réellement disponibles dans la boutique.</p><Link href={article.cta} className="mt-6 inline-flex items-center gap-2 rounded-full bg-slate-950 px-5 py-3 text-sm font-bold text-white hover:bg-[#1672d8]">{article.ctaLabel}<ArrowRight size={16} /></Link></section></div><aside className="h-fit rounded-[1.7rem] border border-slate-200 bg-white p-6 lg:sticky lg:top-24"><div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-blue-50 text-[#1672d8]"><CircleHelp size={19} /></div><h2 className="mt-5 font-display text-xl font-extrabold">Checklist avant achat</h2><ul className="mt-5 space-y-3">{article.checklist.map((item) => <li key={item} className="flex gap-2.5 text-sm leading-6 text-slate-600"><Check size={16} className="mt-1 shrink-0 text-[#1672d8]" />{item}</li>)}</ul></aside></div><section className="mt-14 border-t pt-10"><h2 className="font-display text-2xl font-extrabold">Des modèles à examiner</h2><p className="mt-3 text-sm leading-6 text-slate-600">Sélection éditoriale du catalogue, à comparer selon la configuration souhaitée.</p><div className="mt-6 grid gap-5 sm:grid-cols-2">{dashcams.filter(p => slug === "dashcam-moto-guide" ? /moto/i.test(p.productType) : (slug === "dashcam-voiture-discrete" ? ["dashcam-voiture-discrete-2k","dashcam-2k-voiture","dashcam-avant-arriere"] : ["dashcam-3k-voiture","dashcam-wifi-5ghz"]).includes(p.handle)).slice(0,3).map(p=><ProductCard key={p.id} product={p} />)}</div><Link href="/quiz" className="btn-secondary mt-6">Trouver un modèle selon mon usage</Link></section><DecisionLinks /></div></article></StorefrontLayout>;
}
