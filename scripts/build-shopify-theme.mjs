...
<script id="norticam-policy-data" type="application/json">{"privacyPolicy":{{ shop.privacy_policy | json | replace: '</', '\\u003c/' }},"refundPolicy":{{ shop.refund_policy | json | replace: '</', '\\u003c/' }},"shippingPolicy":{{ shop.shipping_policy | json | replace: '</', '\\u003c/' }},"termsOfService":{{ shop.terms_of_service | json | replace: '</', '\\u003c/' }}}</script>
